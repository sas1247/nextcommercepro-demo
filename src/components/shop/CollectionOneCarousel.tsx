...
export default function CollectionOneCarousel({ items }: { items: Item[] }) {
  const pageSize = 5;
  const cart = useCart();
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => setIndex((i) => (i + 1) % pages.length), 3000);
    return () => clearInterval(t);
  }, [pages.length]);

  return (
    <section className="max-w-7xl mx-auto px-4 py-6 md:py-10">
      {/* Section header (mobile: smaller, button on a single line) */}
      <div className="flex items-end justify-between gap-3 mb-4 md:mb-5">
        <div className="min-w-0">
          <h2 className="text-lg md:text-2xl font-semibold text-black leading-tight">Featured Collection One</h2>
          <p className="text-xs md:text-sm text-neutral-600 mt-1">Curated picks for your customers</p>
        </div>

        <Link
          href="/categorie/category-four"
          className="shrink-0 inline-flex items-center justify-center rounded-xl
                     ...
                     hover:shadow-lg hover:shadow-[#3533cd]/30
      ...
      {/* ✅ MOBILE: horizontal scroller (2 products + a bit of the 3rd) */}
      ...
      {/* ✅ DESKTOP: carousel (kept intact, only money formatting changed) */}
